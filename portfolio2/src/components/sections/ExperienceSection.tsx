  ...
  return (
    <section id="experience" className="py-32 px-6">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-0 border border-border shadow-sm">
          {/* Timeline */}
          <div className="lg:col-span-2 border-b lg:border-b-0 lg:border-r border-border bg-surface/50">
            {experiences.map((exp, i) => (
              <button
                key={exp.id}
                onClick={() => setActiveId(exp.id)}
                className={`w-full text-left p-6 border-b border-border last:border-b-0 transition-all duration-200 group ${
                  activeId === exp.id ? "bg-accent-light" : "hover:bg-surface"
                }`}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="font-mono text-[0.6rem] text-muted tracking-wider mb-1">{exp.period}</div>
                    <div
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  {active.current && (
                    <span className="px-2 py-0.5 bg-accent-light border border-accent/20 text-accent font-mono text-[0.6rem] tracking-widest uppercase">
                      Present
                    </span>
                  )}
              ...
              <div className="w-12 h-12 border border-border flex items-center justify-center font-display text-xl text-primary shrink-0 bg-surface">
                {active.logoInitial}
              </div>
            </div>
          ...
          <div className="flex overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
            <div className="marquee-track">
              {[
                ...
                >
                  {company}<span className="text-accent ml-6 mr-3">·</span>
                </span>
              ))}
            </div>
          </div>
