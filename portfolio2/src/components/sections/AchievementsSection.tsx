...
export function AchievementsSection() {
  return (
    <section id="achievements" className="py-32 px-6 bg-surface/50">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {achievements.map((a) => (
            <div
              key={a.id}
              className="group relative border border-border hover:border-accent/40 bg-white p-8 transition-all duration-300 overflow-hidden shadow-sm hover:shadow-md"
            >
              {/* Corner accent */}
              <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div
                  className="absolute top-0 right-0 w-32 h-32 bg-accent-light"
        ...
        {/* IEEE highlight */}
        <div className="border border-accent/25 bg-accent-light p-10 relative overflow-hidden">
          <div
            className="absolute right-0 top-0 bottom-0 w-1/3 pointer-events-none"
            style={{
              background: "radial-gradient(ellipse at right center, rgba(26,107,60,0.08) 0%, transparent 70%)",
            }}
          />
